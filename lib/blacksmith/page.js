...
Page.prototype.renderContent = function (source, contentOptions) {
  ...
  if (!contentOptions || typeof contentOptions === 'string') {
    ...
    if (source[plural]) {
      tree = Object.keys(source[plural]).reduce(function (fullTree, key) {
        fullTree[key] = Object.keys(source[plural][key])
          .filter(function (childKey) {
            return childKey[0] !== '_';
          })
          .map(function (childKey) {
            if (!source[plural][key][childKey] || (!source[plural][key][childKey]._content
              && !source[plural][key][childKey].index)) {
              return;
            }
            else if (source[plural][key][childKey]._content) {
              return source[plural][key][childKey]._content.metadata
            }
            else if (source[plural][key][childKey].index) {
              return source[plural][key][childKey].index._content &&
                source[plural][key][childKey].index._content.metadata;
            }
          })
          .filter(Boolean);
      ...
      return Object.keys(tree).reduce(function (rendered, key) {
        return rendered + partial.render({
          metadata:   {
            tree: tree[key].sort(function (lval, rval) {
              ...
              if (ldetails.source > rdetails.source) return 1;
              if (ldetails.source < rdetails.source) return -1;
              return 0;
            })
          },
          html:       self.html.partials[contentOptions.partial],
          remove:     self.partials[contentOptions.partial] && self.partials[contentOptions.partial].remove,
          references: self.references
        });
      }, '');
    }
