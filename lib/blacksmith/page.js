...
Page.prototype.render = function (key, source) {
  ...
  //
  if (Object.keys(source).length === 2 && source.files && (source['index.md']
    || source['index.markdown'])) {
    content = source['index.md'] || source['index.markdown'];
    content.metadata = content.metadata || {};
    content.metadata['page-details'] = content.metadata['page-details'] || {};
    
    //
    // Remark: These are not normalized. 
    //
    content.metadata['page-details'].files = source.files;
  }
  else {
    content = source;
  }
