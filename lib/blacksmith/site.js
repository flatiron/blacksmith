          ...
          function renderMarkdown(file, next) {
            ...
            fs.stat(fullpath, function (err, stats) {
              ...
              content.render({
                ...
                // create an item in the RSS feed
                feed.item({
                  title: results.metadata['page-details'].title,
                  description: results.truncated.html,
                  url: results.metadata['page-details'].href,
                  author: results.metadata.author,
                  date: rendered[name]._date
                })
...
Site.prototype.writeFiles = function (rendered, callback) {
  ...
  async.series([
    ...
    function writeRSS(next){
      fs.writeFile(path.join(publicDir, 'feed.xml'), feed.xml(), 'utf8', next);
    },
    //
