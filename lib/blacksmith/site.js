            ...
            fs.stat(fullpath, function (err, stats) {
              ...
              content.render({
                ...
                if (name === 'index') {
                  ...
                  rendered._date = results.metadata.date
...
//
Site.prototype.targetFiles = function (rendered) {
  ...
  return Object.keys(rendered).reduce(function (all, key) {
    if (!rendered[key].files) {
      return all.concat({
        fullpath: path.join(publicDir, key + '.html'),
        contents: rendered[key].html
      })
    }
    
    return all.concat([
      {
        fullpath: path.join(publicDir, key),
        contents: 'directory'
      },
      {
        fullpath: path.join(publicDir, key, 'index.html'),
        contents: rendered[key].html
      }
    ]).concat(rendered[key].files.map(function (file) {
      return {
        fullpath: path.join(publicDir, key, path.basename(file)),
