...
// Attaching nconf to the docs object for later use.
docs.config = nconf;
...
docs.generate = exports.generate = function (src, dst) {
  ...
  // Attaching these to docs for future use
  ...
  jsdom.env( "<html><body></body></html>", ['./jquery.js'], function (err, window) {
     ...
     try {
       docs.generators.forEach(function(_generator, n){
         ...
         //
         // Note: jsdom strips doctypes.
         // given the dom, you should be able to access the doctype, but
         // because we're using the div to contain our document this 
         // information never gets added to jsdom.
         // This works because I set the doctype for the jsdom explicitly.
         generator.generate(
           docs.window.document.doctype
           + "\n"
           + div.innerHTML, docs.content[label]
         );
       });
     } catch (err) {
