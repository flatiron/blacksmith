...
fs2.filesToTree = function(files){
  ...
  files = files.reduce(function (acc, file) { 
    ...
    // Use traverse to set acc[p[0]][p[1]]...[p[n-1]] to {}, *if* this path does
    // not already exist. Traverse takes care of creating necessary objects.
    if (!traverse(acc).get(ps)) {
      traverse(acc).set(ps, {});
    }
    return acc;
  }, {});
...
//     { "foo": {"bar": {} } }
//
fs2.dirToTree = function (dir, opts) {

  var ls = Object.keys(dir);

  if (opts.root) {
    ls = ls.map(function (p) {
      return helpers.unresolve(path.resolve(opts.root+'/../'), p);
    });
  }

  // Same as filesToTree, except we use Object.keys which is equivalent to the
  // original list of files.
  return fs2.filesToTree(ls);
