...
articles.weld = function(dom, pages) {
  ...
  Object.keys(pages).forEach( function (i){
    ...
    if (pages[i].content) {
      var content;
      try {
        content = markdown.parse(pages[i].content.toString());
      } catch (err) {
        content = err.message;
      }
    }
    ...
    pages[i].content = dom.innerHTML || docs.content.theme['./theme/article.html'].toString();

  });
  return dom;
};

articles.generate = function(output, pages) {
...
// Load all content with an fs2.readDirSync. It should be relatively indiscriminate.
