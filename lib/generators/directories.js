...
// TODO: port to use fs2.js
dir.load = function() {
  // TODO: use/test docs.src
  var _dir = findit.sync('./pages');

  var dir = {};

  // Grab all folders that *only* have more folders
  // or explicitly have a metadata.json that specifies
  // "directory"
  _dir.filter(function(dir){

    if (fs.statSync(dir).isDirectory()) {

      // check to see if the option is explicitly set
      if (path.existsSync(dir+"/metadata.json") && fs.statSync(dir+"/metadata.json").isFile()) {
        return JSON.parse(fs.readFileSync(dir+"/metadata.json").toString()).generator === "directory";
      }
