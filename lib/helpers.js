...
helpers.buildToc = function () {
  ...
  // TODO: Needs to work with docs.src
  var src = "./pages";
...
helpers.tocSort = function(toc) {
  return toc.map(function sortToc (row, paths) {
    ...
    // Fails if there's no page.json
    // todo: set page.json to a variable
    try {
      var order = JSON.parse(fs.readFileSync(paths.join("/")+"/../page.json").toString()).order;
...
helpers.articlesToObject = function (src, files) {
  ...
  //Hard-coded content and page.json here.
  //Is this helper still being used?

  Object.keys(files).forEach(function (f) {
    var pathName = path.dirname(f) + "/content.md";
...
helpers.dirToContent = function (src, files, resolve) {
  ...
  Object.keys(files).forEach(function (f) {
    ...
    // if it's a page.json, load the metadata.
    if (path.basename(f) === "page.json") {
      try {
    ...
    } // grab content---assumes content is in "content.md" for now
    else if (path.basename(f) === "content.md") {
      if (!content[p]) {
        content[p] = {};
      }
