  ...
  pages.forEach(function (file) {
    ...
    var data = {
      article: sortedKeys.map( function (i) {
        ...
        // "Render" the title (with link), so it can be consumed later by weld.
        if (!/^<a/.test(content[i].metadata.title)) {
          content[i].metadata.title = [
            '<a href="', helpers.unresolve(smith.src, content[i].metadata.link), '">',
    ...
    weld(dom, data, {
      map: function (parent, element, key, val) {
        ...
        // Populate the title and associated href
