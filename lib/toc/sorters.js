...
exports.byDate = function (subtoc, filename) {
  ...
  // This fs-reading is annoying, but I'm stuck doing it until I rewrite the 
  // core toc sort algorithm. Talk about debt!
  var order = {};

  fs.readdirSync(filename).filter(function (fn) {
    return fs.statSync(filename + "/" + fn).isDirectory();
  }).filter(function (fn) {
    console.log(filename + "/" + fn + "/page.json");
    return path.existsSync(filename + "/" + fn + "/page.json");
  }).map(function (fn) {
    var res;
    try {
      ...
      smith.log.warn("Having trouble reading "+filename + "/" + fn+"/page.json");
      smith.log.warn(JSON.stringify(e.stack, true, 2));
    ...
    if (res[1].toDateString() == "Invalid Date") {
      smith.log.warn(filename + "/" + fn + "/page.json contains an invalid datestring!");
