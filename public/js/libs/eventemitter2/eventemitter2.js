...
;!function(root) {
  ...
  var EventEmitter2 = root.EventEmitter2 = function EventEmitter2(conf) {
    // conf = conf || {};
    // if (conf.delimiter === '*') this.setEventDelimiter(conf.delimiter);
    // this._eventCaseSensitive = conf.eventCaseSensitive;
    // this._eventDelimiter = conf.eventDelimiter || '.';
  ...
  EventEmitter2.prototype.addListener = function(event, listener) {
    ...
    if (~event.indexOf(delimiter)) {
      ...
      // PERFORMANCE???
      if (ns.some(function(event) { return event === ''; })) {
        me.eventNameError('addListener');
        return false;
      }
  ...
  EventEmitter2.prototype.emit = function(event) {
    ...
    // Loop through the collected listeners and invoce them.
    for (i = 0, l = collectedListeners.length; i < l; i++) {
      collectedListeners[i].apply(me, args);
      invoked = true;
    }

    // If a listener "all" is defined in *events* invoce its functions.
